...
*Fig. `r figno()`: Lengths of contigs (fasta records) for each sample. The colors have no intrinsic meaning, other than highlighting the decreasing size of contigs/records in assemblies.*



## Genome annotation

Computed using the [Prokka](https://github.com/tseemann/prokka) genome annotator:
...
Mlst automatically detects the best scheme for typing. If you don't agree with the automatic detection, you can enforce a scheme by running assemblycomparator2 with `--config mlst_scheme=hpylori`. Replace *hpylori* with the mlst scheme you wish to use. You can find a full list of schemes in the mlst [documentation](https://github.com/tseemann/mlst#available-schemes).
...
Resistance genes provided by The Comprehensive Antibiotic Resistance Database ([Card](https://card.mcmaster.ca/)) via abricate.
...
Plasmid calls provided by [PlasmidFinder](https://cge.cbs.dtu.dk/services/PlasmidFinder/) via abricate.
...
Virulence factor calls provided by The virulence factor database ([VFDB](http://www.mgc.ac.cn/VFs/main.htm)) via abricate.
...
The core genome phylogeny is provided with [FastTree](http://www.microbesonline.org/fasttree/) with the generalized time-reversible (GTR) model of nucleotide evolution. The plotted tree is not rooted.
...
*Fig. `r figno()`: Phylogenetic NJ-tree from the concatenated core genome of all samples.*


SNP-distances between the core gene alignment. Calculated with [snp-dists](https://github.com/tseemann/snp-dists#snp-dists). The shown table is symmetrical around the diagonal.
...
```
*Table `r tableno()`: Pairwise distances between the samples in the core genome.*
