...
setwd("~/assemblycomparator2/tests/E._faecium_plasmids/output_asscom2/")    
...
tryCatch({
    ...
    gc_summary_file = "collected_results/GC_summary.tsv"
        ...
        assembly_stats_GC = gc_summary_df %>% 
            ...
            left_join(
                gc_summary_df %>% 
                    ...
                    summarize(sd(GC)),
                by = "filename") %>% 
            mutate_at(vars(GC, `sd(GC)`), signif, digits = 3)
...
try({
    # Now when the figure height has been set, we can generate the figure
    prokka_summarized_file = "collected_results/prokka_summarized.txt"
...
KEGG Orthology is a curated database of most known orthologous genes. The ortholog grouping makes it possible to apply knowledge about genes from one species to other species. In this analysis we ask the question; whether more genes from a specific pathway are present compared to what we expect by chance. The p-value tells us what the probability is, of observing the present genes from each pathway, given that the genomes are randomly generated. The p-values are corrected for false discovery rate for each sample. Only significant results are shown.
...
sample_pathway_enrichment_analysis_file = "collected_results/sample_pathway_enrichment_analysis.tsv"
...
try({
    fig_height = max(4,
                     (sample_pathway_enrichment_analysis_df$description %>% unique() %>% length()) * 0.4)
...
sample_pathway_enrichment_analysis_df %>% 
    ggplot(aes(sample, description, fill = `p.adjust`, label = generatio)) +
    geom_tile() + 
...
```
*Fig. `r figno()`: KEGG pathways which are significantly enriched for in each full genome. The p-values are fdr-adjusted for each sample independently. The text on the tiles (k/n) are the clusterProfiler geneRatios: k is the overlap between the genes in the genome and the genes in the pathway. n is the overlap between genes in the genome and all genes in the collected pathways of the KEGG database. Results are obtained using [clusterProfiler](https://github.com/YuLab-SMU/clusterProfiler).*
...
The [assemblycomparator2](https://github.com/cmkobel/assemblycomparator2#assemblycomparator2) pipeline and report is developed by [Oliver Hansen](https://github.com/oliverh2020) & [Carl M. Kobel](https://github.com/cmkobel) `version 2.2.0`
