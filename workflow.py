...
#gwf.target_from_template(sanify('cmp_kraken2_abricateall_' + title), kraken2_table(target_dir, title, names))
...
gwf.target_from_template(sanify('cmp_roary_' + title), roary(target_dir, title, annotations, blastp_identity = 90, allow_paralogs = True))
