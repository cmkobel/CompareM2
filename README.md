...
What is unique about Assemblycomparator2 is that it works strictly downstream of assembling and binning. Many other tools also include all the steps necessary to turn raw reads into genome representatives, and then does varying degrees of biological characterization of these freshly created assemblies/bins/genomes. It is a conscious decision to exclude the raw read-dependent tools out of the equation for Assemblycomparator2. This is because read-mapping, assembling or even binning is highly dependent on the sequencing technology used and requires a highly specialized pipeline for each technological use case. Assemblycomparator2 takes a different approach which is that you should be able to source your genomes from anywhere and compare them, regardless of the sequencing technology used to create them in the first place. Genome quality is only increasing and in the future we will not have to be worried when comparing pyrosequencing and single-molecule sequencing or hybrid approach based genomes in the same batches of Assemblycomparator2. 

Below we are listing some competing pipelines that partly overlap with the use cases of Assemblycomparator2. Although they might be very different from Assemblycomparator2, they might help making a world with higher awareness of microbial diversity.
