...
def render(registry: Registry, selected: list[str] | None, workdir: Path,
    ...
    # Downloads run wherever Snakemake is, not wherever the jobs go. Under a
    # cluster profile every rule is submitted by default, and a compute node is
    # the one machine that may have no outbound network: on GenomeDK a fetch
    # from a node dies with `CondaHTTPError: HTTP 000 CONNECTION FAILED`, and
    # it would take the 60.8 GB GTDB rule with it. The login node has the
    # network, and a download is I/O rather than compute, so it belongs there
    # whatever the executor. With no profile this changes nothing.
    downloads = [db.rule for db in registry.databases(selected)]

    header = [
        "# Generated by comparem2 — do not edit.",
        "# Every rule below is derived from src/comparem2/catalogue.py.",
        "",
        *([f"localrules: {', '.join(downloads)}", ""] if downloads else []),
        "rule all:",
        "    input:",
