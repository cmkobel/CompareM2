...
def test_hand_written_docs_name_real_tools_in_until_examples():
    """The prose pages are not generated, so their `--until` examples drift.

    All three copies of the "everything except the 60.8 GB GTDB download"
    command were wrong at once: every one omitted `biosynthesis` and two also
    omitted `carveme`, so a reader pasting it silently got fewer analyses than
    the sentence promised.
    """
    root = Path(__file__).resolve().parents[2]
    every = [t.name for t in CATALOGUE]
    without_gtdb = sorted(n for n in every if n != "gtdbtk")

    for page in sorted((root / "docs").glob("*.md")):
        text = page.read_text()
        if "Generated by docs/generate.py" in text:
            continue
        for block in re.findall(r"--until((?:[ \t]+[\w.-]+|[ \t]*\\\n)+)", text):
            # `--until TOOL...` in the options table is a metavariable, not a
            # tool; real tool names are lower-case.
            named = [w for w in block.replace("\\", "").split()
                     if w and w == w.lower()]
            unknown = [n for n in named if n not in every]
            assert not unknown, f"{page.name}: not tools: {unknown}"
            # A list this long is the "all but GTDB-Tk" example, and the whole
            # point of it is to be complete.
            if len(named) >= 10:
                assert sorted(named) == without_gtdb, (
                    f"{page.name}: missing {sorted(set(without_gtdb) - set(named))}")
