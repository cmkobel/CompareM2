...
rule get_ncbi_annotation:
    ...
    shell: """
        ...
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/genomic.gff {output.gff_nofasta}
        
        # Make a prokka-like gff containing the contigs after a ##FASTA line. 
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/genomic.gff {output.gff}
        echo "##FASTA" >> {output.gff}
        cat {input.assembly} >> {output.gff}
        
        
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/protein.faa {output.faa}
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/sequence_report.jsonl {output.log}
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/cds_from_genomic.fna {output.ffn}
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/genomic.gtf {output.tsv}
        cp {output_directory}/.ncbi_cache/accessions/{wildcards.sample}/genomic.gbff {output.gbk} # Not sure if it is problematic to have a "flat file". Let's see what happens.
