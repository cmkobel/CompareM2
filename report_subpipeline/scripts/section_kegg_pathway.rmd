...
```{r kegg_pathway_figure, echo=FALSE, message=F, warning=F, error = T, fig.width = max(2, (N_kegg_groups*0.2)+2.5), fig.height = max(8, (N*0.2)+3)}
...
metadata_translate %>% 
    ...
    ggplot(aes(hierarchy, reorder(sample, desc(sample)), fill = n)) + 
