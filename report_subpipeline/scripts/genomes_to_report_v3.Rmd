...
Whether each analysis in assemblycomparator is run depends on the presence of valid output files.
...
section_definition_table = tribble(
    ...
    #"sequence_lengths", 99, Sys.glob(paste0(results_directory, "/assembly-stats/assembly-stats.tsv")),
...
The table above summarizes the number of results files to be processed for each analysis. 




```{r child = paste0(report_scripts, "section_assembly_stats.rmd")}
```


```{r child = paste0(report_scripts, "section_sequence_lengths.rmd")}
```


```{r child = paste0(report_scripts, "section_prokka.rmd")}
