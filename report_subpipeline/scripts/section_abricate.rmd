...
# I know there is a summary, but making sure that everything is parsed as numbers is just easier if I do my own pivot.
abricate_card = read_tsv("results_ac2/abricate/card_detailed.tsv", na = ".") %>%
    #rename(file_long = `#FILE`) #%>%
    mutate(file = basename(`#FILE`)) %>%
    select(file, GENE, `%COVERAGE`) %>%
...
# I know there is a summary, but making sure that everything is parsed as numbers is just easier if I do my own pivot.
abricate_plasmidfinder = read_tsv("results_ac2/abricate/plasmidfinder_detailed.tsv", na = ".") %>%
    #rename(file_long = `#FILE`) #%>%
    mutate(file = basename(`#FILE`)) %>%
    select(file, GENE, `%COVERAGE`) %>%
...
# I know there is a summary, but making sure that everything is parsed as numbers is just easier if I do my own pivot.
abricate_vfdb = read_tsv("results_ac2/abricate/vfdb_detailed.tsv", na = ".") %>%
    #rename(file_long = `#FILE`) #%>%
    mutate(file = basename(`#FILE`)) %>%
    select(file, GENE, `%COVERAGE`) %>%
