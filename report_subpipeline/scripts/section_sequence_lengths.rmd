...
```{r sequence_lengths-table, echo=FALSE, message=F, warning=F, error = T, fig.height = max(1.8, (N*0.2)+0.3), fig.width = 10}
...
sequence_lengths_meta_df %>%
    ...
    ggplot(aes(length, reorder(sample_and_GC, desc(sample_and_GC)), fill = GC)) + 
