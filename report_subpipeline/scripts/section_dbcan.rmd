...
df_dbcan %>% 
    ...
    ggplot(aes(substrate_unique_paste, reorder(sample, index), fill = count)) + 
    geom_tile() +
...
*Table `r tableno()`: Overview of carbohydrate active enzymes (Cazymes). The count (color) signifies the number of cazymes in each sample, that can degrade each of the listed substrates. Called using [run_dbcan](https://github.com/linnabrown/run_dbcan).*
