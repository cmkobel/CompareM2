...
```{r busco-plot, echo = F, message = F, warning = F, error = T, fig.height = max(1.8, N*0.2), fig.width = 10}

busco_df_meta %>%
    select(sample, `C&S` = S, `C&D` = D, F, M) %>%
    pivot_longer(-sample) %>%
    mutate(name = factor(name, levels = c("F", "M","C&D", "C&S"))) %>%
    ggplot(aes(value, reorder(sample, desc(sample)), fill = name)) + 
...
*Fig. `r figno()`: BUSCO results visualized. Legend: C&S: Complete and single-copy; C&D: Complete and duplicated; F: Fragmented; M: Missing. For each sample, only the best lineage match (in terms of completeness) is shown.*
