...
# setwd("~/GenomeDK/assemblycomparator2/tests/E._faecium_plasmids/results_ac2/")    

#setwd("/cluster/projects/nn9864k/cmkobel/assemblycomparator2/tests/E._faecium")     # Development
#setwd("/cluster/home/cmkobel/memokobel/15_bio326/synthetic_rumen/output/metabat2/06_compare_best_with_lives")     # Development

# axodines
# /home/axodines/assemblycomparator2/conda_base/7747644331801b571bf2c3eb25e862b1_
setwd("/cluster/home/cmkobel/axodines/assemblycomparator2/tests/MAGs")     # Development
#getwd()
...
metadata_file = "results_ac2/metadata.tsv"
...
tryCatch({
    assembly_stats_file = "results_ac2/assembly-stats/assembly-stats.tsv"
    gc_summary_file = "results_ac2/collected_results/GC_summary.tsv"
...
try({
    # Now when the figure height has been set, we can generate the figure
    prokka_summarized_file = "results_ac2/collected_results/prokka_summarized.txt"
...
```{r echo=FALSE, message=F, warning=F}
try({
    busco_extract_file = "results_ac2/collected_results/busco.tsv"
    if (file.exists(busco_extract_file)) {
        ...
        busco_df_meta = metadata_translate %>%
            ...
            left_join(busco_df %>% filter(rank == 1), by = "filename_short") %>%
            select(-filename_short, -filename, -rank) %>%
            arrange(lineage)

        busco_df_meta %>% 
            write_tsv("results_ac2/collected_results/export_busco.tsv")
        ...
        #             dom = 'Blfrtip',
        #             buttons = c('csv', 'excel'),
        #             #buttons = c(list(extend = "csv", filename = "junk")),
        #             lengthMenu = list(
        #                 c(10,25,50,-1),
        #                 c(10,25,50, "All")
        #             )
...
try({
    ...
    if (file.exists(busco_extract_file)) {
        ...
        busco_raw = read_delim(busco_extract_file, delim = ": ", col_names = c("name", "value"))
        ...
        busco_df_meta = metadata_translate %>%
            ...
            left_join(busco_df %>% filter(rank == 1), by = "file") %>%
            select(-file, -filename, -rank) %>%
            arrange(lineage)
        busco_df_meta
...
```{r echo=FALSE, message=F, warning=F, fig.height = 4.8, fig.width = 8}
try({
    if (file.exists(busco_extract_file)) {

        busco_df_meta %>% 

            ggplot(aes(C, F, color = lineage, label = sample)) + 
            geom_point() +
...
*Fig. `r figno()`: BUSCO Completeness VS Fragmentation. Axes are in percents. Colored by lineage. For each sample, only the best lineage match (in terms of completeness) is shown. Samples in the lower right corner are relatively better.*
...
```{r echo=FALSE, message=F, warning=F}
try({
    checkm2_file = "results_ac2/checkm2/quality_report.tsv"
    if (file.exists(checkm2_file)) {

        # Read raw input
        checkm2_raw = read_tsv(checkm2_file)
        ...
        paste("Warning: The file", checkm2_file, "is not accessible.")
    }
...
```{r echo=FALSE, message=F, warning=F}
try({
    kraken2_file = "results_ac2/collected_results/kraken2_reports.tsv"
...
```{r echo=FALSE, message=F, warning=F}
try({
    gtdbtk_file = "results_ac2/gtdbtk/gtdbtk.bac.summary.tsv"
...
try({
    ...
    mlst_file = "results_ac2/mlst/mlst.tsv"
    if (file.exists(mlst_file)) {
        mlst_df <- read_tsv("results_ac2/mlst/mlst.tsv", col_names = F)
...
```{r echo=FALSE, message=F, warning=F}
try({
    abricate_ncbi_file = "results_ac2/abricate/ncbi_summarized.tsv"
...
```{r echo=FALSE, message=F, warning=F}
try({
    abricate_card_file = "results_ac2/abricate/card_summarized.tsv"
...
```{r echo=FALSE, message=F, warning=F}
try({
    abricate_plasmidfinder_file = "results_ac2/abricate/plasmidfinder_summarized.tsv"
...
```{r echo=FALSE, message=F, warning=F}
sample_pathway_enrichment_analysis_file = "results_ac2/collected_results/sample_pathway_enrichment_analysis.tsv"
...
```{r echo=FALSE, message=F, warning=F}
try({
    fasttree_file = "results_ac2/fasttree/fasttree.newick"
...
try({
    snp_dists_file = "results_ac2/snp-dists/snp-dists.tsv"
...
```{r echo=FALSE, message=F, warning=F}
try({
    mashtree_file = "results_ac2/mashtree/mashtree.newick"
