    ...
    if (file.exists(sequence_lengths_file)) {
        sequence_lengths_df = read_tsv(sequence_lengths_file) %>%
            ...
            mutate(init = lag(length, default = 0),
                   start = cumsum(init),
                   stop = start + length - 1) %>% 
            # mutate(sample_ = fct_reorder(sample, desc(factor(sample)))) %>% 
            # mutate(sample__ = factor(sample, levels = sample %>% unique %>% sort(decreasing = T))) %>% 
            # mutate(sample___ = fct_rev(factor(sample))) %>% 
            # mutate(sample____ = reorder(sample, desc(sample))) %>% 
            # pull(sample____)
            identity()
        
        
        sequence_lengths_df_ = sequence_lengths_df %>% 
            left_join(ribosomes, by = c("sample", "record")) %>% 
            mutate(plot_start = start + r_start,
                   plot_end = stop + r_end) %>% 
            mutate(target = str_extract(r_product %>% str_trim(), "^\\d+|DnaA$"))# %>% 
            # filter(S_ribosomal_RNA == "16")
        ...
        sequence_lengths_df %>%         
            ...
            #ggplot(aes(length, sample, fill=reorder(col, desc(col)))) + 
            ...
            geom_point(mapping = aes(x = plot_start), data = sequence_lengths_df_ %>% filter(str_detect(target, "16")), shape = 5, alpha = 0.7, size = 1) +
            geom_point(mapping = aes(x = plot_start), data = sequence_lengths_df_ %>% filter(str_detect(target, "DnaA")), shape = 4, alpha = 0.7, size = 2) +
