...
try({
    ...
    if (file.exists(busco_extract_file)) {
        ...
        busco_df_meta %>% 
            write_tsv("output_asscom2/collected_results/export_busco.tsv")
...
*Fig. `r figno()`: BUSCO Completeness VS Fragmentation. Axes are in percents. Colored by lineage. For each sample, only the best lineage match (in terms of completeness) is shown.*
...
```{r echo=FALSE, message=F, warning=F}
try({
    checkm2_file = "output_asscom2/checkm2/quality_report.tsv"
...
*Fig. `r figno()`: [Checkm2](https://github.com/chklovski/CheckM2) results. Axes are in percents. Colored by coding density.*
